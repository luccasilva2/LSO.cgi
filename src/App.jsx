...
function App() {
    ...
    // Face auth removed: app is always unlocked.
    ...
    useEffect(() => {
        ...
        if (isConnected && isAuthenticated && socketConnected && micDevices.length > 0 && !hasAutoConnectedRef.current) {
            ...
            const timer = setTimeout(() => {
                ...
                console.log("Auto-connecting to model with device:", deviceName);
    ...
    useEffect(() => {
        ...
        socket.on('status', (data) => {
            ...
            if (data.msg === 'LSO Started' || data.msg === 'A.D.A Started') {
                setStatus('Model Connected');
            } else if (data.msg === 'LSO Stopped' || data.msg === 'A.D.A Stopped') {
                setStatus('Connected');
            }
        });
    ...
    const togglePower = () => {
        if (isConnected) {
            ...
            const queryDevice = micDevices.find(d => d.deviceId === selectedMicId);
            const deviceName = queryDevice?.label?.trim() || null;
            socket.emit('start_audio', { device_name: deviceName, muted: false });
            setIsConnected(true);
    ...
    return (
        <div className="app-shell h-screen w-screen text-cyan-100 overflow-hidden flex flex-col relative selection:bg-cyan-900 selection:text-white">
            ...
            <div className="absolute inset-0 app-bg-grid z-0 pointer-events-none opacity-65"></div>
            <div className="absolute inset-0 app-bg-noise z-0 pointer-events-none opacity-20"></div>
            <div className="absolute top-1/2 left-1/2 w-[860px] h-[860px] rounded-full bg-cyan-400/10 blur-[140px] pointer-events-none app-orb"></div>

            {/* Top Bar (Draggable) */}
            <div className="z-50 flex items-center justify-between p-2 border-b border-cyan-200/10 bg-black/35 backdrop-blur-md select-none sticky top-0" style={{ WebkitAppRegion: 'drag' }}>
                <div className="flex items-center gap-3 pl-2">
                    <h1 className="text-xl font-bold tracking-[0.24em] text-cyan-100 drop-shadow-[0_0_10px_rgba(74,217,209,0.45)]">
                        LSO
                    </h1>
                    <div className="ui-mono text-[10px] text-cyan-300/90 border border-cyan-300/20 bg-cyan-400/10 px-2 py-0.5 rounded">
                        V2.1
                    </div>
                    <div className="ui-mono text-[10px] text-emerald-200/80 border border-emerald-300/20 bg-emerald-500/10 px-2 py-0.5 rounded">
                        {status.toUpperCase()}
                    </div>
                    {/* FPS Counter */}
                    {isVideoOn && (
                        <div className="ui-mono text-[10px] text-green-300 border border-green-500/30 bg-green-500/10 px-2 py-0.5 rounded ml-1">
                            FPS: {fps}
                        </div>
                    )}
                ...
                <div className="flex items-center gap-2 pr-2" style={{ WebkitAppRegion: 'no-drag' }}>
                    {/* Live Clock */}
                    <div className="flex items-center gap-1.5 text-[11px] text-cyan-200/80 ui-mono px-2">
            ...
            <div className="flex-1 relative z-10 flex flex-col items-center justify-center">
                ...
                <div className="absolute top-[70px] left-1/2 -translate-x-1/2 text-cyan-100/85 text-xs ui-mono tracking-[0.22em] pointer-events-none z-50 bg-black/45 px-3 py-1 rounded backdrop-blur-sm border border-cyan-200/15">
