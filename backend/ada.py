...
config = types.LiveConnectConfig(
    ...
    system_instruction="Your name is LSO (Logic Support Operator). "
...
class AudioLoop:
    def __init__(self, video_mode=DEFAULT_MODE, on_audio_data=None, on_video_frame=None, on_cad_data=None, on_web_data=None, on_transcription=None, on_tool_confirmation=None, on_cad_status=None, on_cad_thought=None, on_project_update=None, on_device_update=None, on_error=None, input_device_index=None, input_device_name=None, output_device_index=None, kasa_agent=None):
        ...
        # Echo suppression state (prevents LSO voice from re-entering mic path).
        self.echo_suppression_enabled = True
        self.echo_suppression_window_sec = 0.45
        self.echo_suppression_rms_override = 2600
        self._last_output_audio_time = 0.0
        self._last_echo_suppression_log = 0.0

        # Strict anti-feedback mode: mic is blocked while LSO is speaking
        # and for a short cooldown afterwards (half-duplex behavior).
        self.half_duplex_enabled = True
        self.half_duplex_cooldown_sec = 1.1
        self._tts_block_until = 0.0
    ...
    def update_permissions(self, new_perms):
        print(f"[LSO DEBUG] [CONFIG] Updating tool permissions: {new_perms}")
    ...
    def resolve_tool_confirmation(self, request_id, confirmed):
        print(f"[LSO DEBUG] [RESOLVE] resolve_tool_confirmation called. ID: {request_id}, Confirmed: {confirmed}")
        if request_id in self._pending_confirmations:
            future = self._pending_confirmations[request_id]
            if not future.done():
                print(f"[LSO DEBUG] [RESOLVE] Future found and pending. Setting result to: {confirmed}")
                future.set_result(confirmed)
            else:
                 print(f"[LSO DEBUG] [WARN] Request {request_id} future already done. Result: {future.result()}")
        else:
            print(f"[LSO DEBUG] [WARN] Confirmation Request {request_id} not found in pending dict. Keys: {list(self._pending_confirmations.keys())}")
    ...
    def clear_audio_queue(self):
        ...
        try:
            ...
            while not self.audio_in_queue.empty():
                ...
                count += 1
            if count > 0:
                print(f"[LSO DEBUG] [AUDIO] Cleared {count} chunks from playback queue due to interruption.")
        except Exception as e:
            print(f"[LSO DEBUG] [ERR] Failed to clear audio queue: {e}")
    ...
    async def listen_audio(self):
        def _norm_name(name):
            if not name:
                return ""
            out = name.lower()
            for token in ["(input)", "(output)", "default", "built-in", "microphone", "mic"]:
        ...
        def _looks_like_loopback(name):
            ...
            return any(token in n for token in loopback_tokens)

        # Enumerate input-capable devices once for robust matching and debugging.
        input_devices = []
        ...
        if not input_devices:
            print("[LSO] [ERR] No input-capable audio devices found by PortAudio.")
            return

        preferred_input_devices = [(idx, name) for idx, name in input_devices if not _looks_like_loopback(name)]
        if preferred_input_devices:
            print(f"[LSO] Non-loopback input candidates: {len(preferred_input_devices)}/{len(input_devices)}")
        else:
            print("[LSO] [WARN] No clear non-loopback mic found; using any available input device.")

        print("[LSO] Input devices detected:")
        for idx, name in input_devices[:12]:
            print(f"   {idx}: {name}")

        resolved_input_device_index = None

        # 1) Prefer matching by device name (frontend/browser device label).
        if self.input_device_name and str(self.input_device_name).strip():
            requested = str(self.input_device_name).strip()
            requested_norm = _norm_name(requested)
            print(f"[LSO] Attempting to find input device matching name: '{requested}'")

            # Exact normalized match first
            for idx, name in preferred_input_devices:
                if _norm_name(name) == requested_norm:
                    resolved_input_device_index = idx
                    break
            ...
            if resolved_input_device_index is None:
                for idx, name in preferred_input_devices:
                    n = _norm_name(name)
                    if requested_norm and (requested_norm in n or n in requested_norm):
                        resolved_input_device_index = idx
                        break

            if resolved_input_device_index is not None:
                chosen_name = dict(input_devices).get(resolved_input_device_index, "unknown")
                print(f"[LSO] Resolved input device '{requested}' -> index {resolved_input_device_index} ({chosen_name})")
            else:
                print(f"[LSO] Could not match '{requested}'. Falling back to default input device.")

        # 2) Optional numeric fallback, only if it points to an input-capable device.
        if resolved_input_device_index is None and self.input_device_index is not None:
            try:
                requested_idx = int(self.input_device_index)
                valid_indices = {idx for idx, _ in preferred_input_devices} or {idx for idx, _ in input_devices}
                if requested_idx in valid_indices:
                    resolved_input_device_index = requested_idx
                    print(f"[LSO] Using provided input device index: {requested_idx}")
                else:
                    print(f"[LSO] Ignoring invalid/non-input device index: {requested_idx}")
            except ValueError:
                print(f"[LSO] Invalid device index '{self.input_device_index}'.")

        # 3) Final fallback: PortAudio default input device, else first available.
        if resolved_input_device_index is None:
            try:
                mic_info = pya.get_default_input_device_info()
                default_idx = int(mic_info["index"])
                default_name = mic_info.get("name", "")
                if _looks_like_loopback(default_name) and preferred_input_devices:
                    resolved_input_device_index = preferred_input_devices[0][0]
                    print(f"[LSO] Default input looked like loopback ('{default_name}'). Using mic index: {resolved_input_device_index}")
                else:
                    resolved_input_device_index = default_idx
                    print(f"[LSO] Using default input device index: {resolved_input_device_index}")
            except Exception:
                fallback_pool = preferred_input_devices if preferred_input_devices else input_devices
                resolved_input_device_index = fallback_pool[0][0]
                print(f"[LSO] No default input device reported. Using first available: {resolved_input_device_index}")
        ...
        try:
            ...
            print(f"[LSO] [ERR] Failed to open audio input stream: {e}")
            print("[LSO] [WARN] Audio features will be disabled. Please check microphone permissions.")
            return
        ...
        while True:
            ...
            try:
                ...
                # 2) Half-duplex hard gate: while TTS is active/recent, block mic frames entirely.
                allow_send = True
                now = time.time()
                if self.half_duplex_enabled and now < self._tts_block_until:
                    allow_send = False
                    if now - self._last_echo_suppression_log > 2.0:
                        print(
                            f"[LSO DEBUG] [HALF_DUPLEX] Mic blocked while LSO speaking "
                            f"(remaining={self._tts_block_until - now:.2f}s)"
                        )
                        self._last_echo_suppression_log = now

                # 3) Soft echo suppression gate (fallback when half-duplex window is over):
                # while LSO was very recently speaking, ignore low-level mic audio.
                if allow_send and self.echo_suppression_enabled:
                    seconds_since_output = time.time() - self._last_output_audio_time
                    is_recent_output = seconds_since_output < self.echo_suppression_window_sec
                    if is_recent_output and rms < self.echo_suppression_rms_override:
                        allow_send = False
                        if now - self._last_echo_suppression_log > 2.0:
                            print(
                                f"[LSO DEBUG] [ECHO] Suppressing mic frame "
                                f"(rms={rms}, recent_output={seconds_since_output:.2f}s)"
                            )
                            self._last_echo_suppression_log = now

                # 4) Send audio to model only if allowed by anti-feedback gates
                if allow_send and self.out_queue:
                    await self.out_queue.put({"data": data, "mime_type": "audio/pcm"})

                if allow_send and rms > VAD_THRESHOLD:
                    # Speech Detected
                    self._silence_start_time = None
                    ...
                    if not self._is_speaking:
                        ...
                        print(f"[LSO DEBUG] [VAD] Speech Detected (RMS: {rms}). Sending Video Frame.")
                        
                        # Send ONE frame
                        if self._latest_image_payload and self.out_queue:
                            await self.out_queue.put(self._latest_image_payload)
                        else:
                            print(f"[LSO DEBUG] [VAD] No video frame available to send.")
                    ...
                    if self._is_speaking:
                        if self._silence_start_time is None:
                            ...
                            print(f"[LSO DEBUG] [VAD] Silence detected. Resetting speech state.")
                            self._is_speaking = False
                            self._silence_start_time = None
    ...
    async def handle_cad_request(self, prompt):
        print(f"[LSO DEBUG] [CAD] Background Task Started: handle_cad_request('{prompt}')")
        if self.on_cad_status:
        ...
        if self.project_manager.current_project == "temp":
            ...
            print(f"[LSO DEBUG] [CAD] Auto-creating project: {new_project_name}")
            ...
            if success:
                ...
                try:
                    ...
                    print(f"[LSO DEBUG] [ERR] Failed to notify auto-project: {e}")
        ...
        if cad_data:
            print(f"[LSO DEBUG] [OK] CadAgent returned data successfully.")
            print(f"[LSO DEBUG] [INFO] Data Check: {len(cad_data.get('vertices', []))} vertices, {len(cad_data.get('edges', []))} edges.")
            
            if self.on_cad_data:
                print(f"[LSO DEBUG] [SEND] Dispatching data to frontend callback...")
                self.on_cad_data(cad_data)
                print(f"[LSO DEBUG] [SENT] Dispatch complete.")
            ...
            try:
                ...
                print(f"[LSO DEBUG] [NOTE] Sent completion notification to model.")
            except Exception as e:
                 print(f"[LSO DEBUG] [ERR] Failed to send completion notification: {e}")

        else:
            print(f"[LSO DEBUG] [ERR] CadAgent returned None.")
    ...
    async def handle_write_file(self, path, content):
        print(f"[LSO DEBUG] [FS] Writing file: '{path}'")
        ...
        if self.project_manager.current_project == "temp":
            ...
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            new_project_name = f"Project_{timestamp}"
            print(f"[LSO DEBUG] [FS] Auto-creating project: {new_project_name}")
        ...
        print(f"[LSO DEBUG] [FS] Resolved path: '{final_path}'")
        ...
        print(f"[LSO DEBUG] [FS] Result: {result}")
        try:
             await self.session.send(input=f"System Notification: {result}", end_of_turn=True)
        except Exception as e:
             print(f"[LSO DEBUG] [ERR] Failed to send fs result: {e}")

    async def handle_read_directory(self, path):
        print(f"[LSO DEBUG] [FS] Reading directory: '{path}'")
        try:
    ...
    async def handle_read_file(self, path):
        print(f"[LSO DEBUG] [FS] Reading file: '{path}'")
        try:
            if not os.path.exists(path):
    ...
    async def handle_web_agent_request(self, prompt):
        print(f"[LSO DEBUG] [WEB] Web Agent Task: '{prompt}'")
        ...
        print(f"[LSO DEBUG] [WEB] Web Agent Task Returned: {result}")
        ...
        try:
             ...
             print(f"[LSO DEBUG] [ERR] Failed to send web agent result to model: {e}")
                            ...
                            if transcript:
                                ...
                                if transcript != self._last_output_transcription:
                                    ...
                                    if delta:
                                        # Send to frontend (Streaming)
                                        if self.on_transcription:
                                             self.on_transcription({"sender": "LSO", "text": delta})
                                        
                                        # Buffer for Logging
                                        if self.chat_buffer["sender"] != "LSO":
                                            # Flush previous
                                            if self.chat_buffer["sender"] and self.chat_buffer["text"].strip():
                                                self.project_manager.log_chat(self.chat_buffer["sender"], self.chat_buffer["text"])
                                            # Start new
                                            self.chat_buffer = {"sender": "LSO", "text": delta}
                                        else:
                                            # Append
                                            self.chat_buffer["text"] += delta
                        ...
                        for fc in response.tool_call.function_calls:
                            if fc.name in ["generate_cad", "run_web_agent", "write_file", "read_directory", "read_file", "create_project", "switch_project", "list_projects", "list_smart_devices", "control_light", "discover_printers", "print_stl", "get_print_status", "iterate_cad"]:
                                ...
                                if not confirmation_required:
                                    print(f"[LSO DEBUG] [TOOL] Permission check: '{fc.name}' -> AUTO-ALLOW")
                                elif self.on_tool_confirmation:
                                    # Confirmation Logic
                                    import uuid
                                    request_id = str(uuid.uuid4())
                                    print(f"[LSO DEBUG] [STOP] Requesting confirmation for '{fc.name}' (ID: {request_id})")
                                    ...
                                    print(f"[LSO DEBUG] [CONFIRM] Request {request_id} resolved. Confirmed: {confirmed}")
                                else:
                                    # No callback available; fail-open to avoid deadlock.
                                    print(f"[LSO DEBUG] [TOOL] No confirmation callback; auto-allowing '{fc.name}'")

                                if not confirmed:
                                    print(f"[LSO DEBUG] [DENY] Tool call '{fc.name}' denied by user.")
                                ...
                                if fc.name == "generate_cad":
                                    print(f"\n[LSO DEBUG] --------------------------------------------------")
                                    print(f"[LSO DEBUG] [TOOL] Tool Call Detected: 'generate_cad'")
                                    print(f"[LSO DEBUG] [IN] Arguments: prompt='{prompt}'")
                                    ...
                                    print(f"[LSO DEBUG] [TOOL] Tool Call: 'run_web_agent' with prompt='{prompt}'")
                                    ...
                                    print(f"[LSO DEBUG] [RESPONSE] Sending function response: {function_response}")
                                    function_responses.append(function_response)
                                    ...
                                    print(f"[LSO DEBUG] [TOOL] Tool Call: 'write_file' path='{path}'")
                                    ...
                                    path = fc.args["path"]
                                    print(f"[LSO DEBUG] [TOOL] Tool Call: 'read_directory' path='{path}'")
                                    ...
                                    path = fc.args["path"]
                                    print(f"[LSO DEBUG] [TOOL] Tool Call: 'read_file' path='{path}'")
                                    ...
                                    name = fc.args["name"]
                                    print(f"[LSO DEBUG] [TOOL] Tool Call: 'create_project' name='{name}'")
                                    ...
                                    name = fc.args["name"]
                                    print(f"[LSO DEBUG] [TOOL] Tool Call: 'switch_project' name='{name}'")
                                    ...
                                    if success:
                                        ...
                                        print(f"[LSO DEBUG] [PROJECT] Sending project context to AI ({len(context)} chars)")
                                        try:
                                            await self.session.send(input=f"System Notification: {msg}\n\n{context}", end_of_turn=False)
                                        except Exception as e:
                                            print(f"[LSO DEBUG] [ERR] Failed to send project context: {e}")
                                    function_response = types.FunctionResponse(
                                        id=fc.id, name=fc.name, response={"result": msg}
                                    )
                                    function_responses.append(function_response)
                                    ...
                                    print(f"[LSO DEBUG] [TOOL] Tool Call: 'list_projects'")
                                    ...
                                    print(f"[LSO DEBUG] [TOOL] Tool Call: 'list_smart_devices'")
                                    ...
                                    print(f"[LSO DEBUG] [TOOL] Tool Call: 'control_light' Target='{target}' Action='{action}'")
                                    ...
                                    print(f"[LSO DEBUG] [TOOL] Tool Call: 'discover_printers'")
                                    ...
                                    print(f"[LSO DEBUG] [TOOL] Tool Call: 'print_stl' STL='{stl_path}' Printer='{printer}'")
                                    ...
                                    printer = fc.args["printer"]
                                    print(f"[LSO DEBUG] [TOOL] Tool Call: 'get_print_status' Printer='{printer}'")
                                    ...
                                    print(f"[LSO DEBUG] [TOOL] Tool Call: 'iterate_cad' Prompt='{prompt}'")
                                    ...
                                    if cad_data:
                                        print(f"[LSO DEBUG] [OK] CadAgent iteration returned data successfully.")
                                        
                                        # Dispatch to frontend
                                        if self.on_cad_data:
                                            print(f"[LSO DEBUG] [SEND] Dispatching iterated CAD data to frontend...")
                                            self.on_cad_data(cad_data)
                                            print(f"[LSO DEBUG] [SENT] Dispatch complete.")
                                        ...
                                        print(f"[LSO DEBUG] [ERR] CadAgent iteration returned None.")
    ...
    async def play_audio(self):
        ...
        while True:
            ...
            now = time.time()
            self._last_output_audio_time = now
            self._tts_block_until = now + self.half_duplex_cooldown_sec
    ...
    async def run(self, start_message=None):
        ...
        while not self.stop_event.is_set():
            try:
                print(f"[LSO DEBUG] [CONNECT] Connecting to Gemini Live API...")
                async with (
                    ...
                    if not is_reconnect:
                        if start_message:
                            print(f"[LSO DEBUG] [INFO] Sending start message: {start_message}")
                        ...
                        print(f"[LSO DEBUG] [RECONNECT] Connection restored.")
                        # Restore Context
                        print(f"[LSO DEBUG] [RECONNECT] Fetching recent chat history to restore context...")
                        ...
                        print(f"[LSO DEBUG] [RECONNECT] Sending restoration context to model...")
                ...
                print(f"[LSO DEBUG] [STOP] Main loop cancelled.")
                break
                
            except Exception as e:
                # This catches the ExceptionGroup from TaskGroup or direct exceptions
                print(f"[LSO DEBUG] [ERR] Connection Error: {e}")
                
                if self.stop_event.is_set():
                    break
                
                print(f"[LSO DEBUG] [RETRY] Reconnecting in {retry_delay} seconds...")
