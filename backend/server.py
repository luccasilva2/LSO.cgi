...
@app.get("/status")
async def status():
    return {"status": "running", "service": "LSO Backend"}

@sio.event
async def connect(sid, environ):
    print(f"Client connected: {sid}")
    await sio.emit('status', {'msg': 'Connected to LSO Backend'}, room=sid)
    # Face authentication disabled globally: always authenticated.
...
async def start_audio(sid, data=None):
    ...
    if audio_loop:
        if loop_task and (loop_task.done() or loop_task.cancelled()):
             ...
             await sio.emit('status', {'msg': 'LSO Already Running'})
             return
    ...
    try:
        ...
        print("Emitting 'LSO Started'")
        await sio.emit('status', {'msg': 'LSO Started'})
        ...
        print(f"CRITICAL ERROR STARTING LSO: {e}")
        import traceback
        traceback.print_exc()
...
async def stop_audio(sid):
    ...
    if audio_loop:
        ...
        audio_loop = None
        await sio.emit('status', {'msg': 'LSO Stopped'})
...
async def iterate_cad(sid, data):
    ...
    if not prompt or not str(prompt).strip():
        await sio.emit('error', {'msg': "Iteration prompt is empty"})
        return
...
async def generate_cad(sid, data):
    ...
    if not prompt or not str(prompt).strip():
        await sio.emit('error', {'msg': "CAD prompt is empty"})
        return
...
async def prompt_web_agent(sid, data):
    ...
    if not prompt or not str(prompt).strip():
        await sio.emit('error', {'msg': "Web Agent prompt is empty"})
        return
    ...
    try:
        ...
        await sio.emit('browser_frame', {'image': None, 'log': f"Final: {result}"})
...
async def discover_printers(sid):
    ...
    if not audio_loop or not audio_loop.printer_agent:
        ...
        if saved_printers:
            ...
            await sio.emit('status', {'msg': "Connect to LSO to enable printer discovery"})
            return
...
async def add_printer(sid, data):
    ...
    raw_host = (data.get('host') or '').strip()
    if not raw_host:
        await sio.emit('error', {'msg': "Printer host/IP is required"})
        return
    ...
    if ":" in raw_host:
        ...
        try:
            ...
            await sio.emit('error', {'msg': f"Invalid printer port: {port_str}"})
            return
    else:
        host = raw_host
        port = 80
...
async def update_settings(sid, data):
    ...
    if "face_auth_enabled" in data:
        # Face auth removed: always keep disabled.
        SETTINGS["face_auth_enabled"] = False
        await sio.emit('auth_status', {'authenticated': True})
